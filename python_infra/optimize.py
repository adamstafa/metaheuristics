...
def run_once(problem, w1, w2, w3):
    problem_path = os.path.join(problems_path, problem)
    proc = os.popen(f'{program_path} {problem_path} {w1} {w2} {w3}')
    result = proc.read().strip()
    return float(result)

def run_multiple(problem, w1, w2, w3, n=3):
    results = []
    for _ in range(n):
        result = run_once(problem, w1, w2, w3)
        results.append(result)
    return sum(results) / len(results)

def evaluate_problems(w1, w2, w3):
    results = []
    for problem in problems:
        result = run_multiple(problem, w1, w2, w3)
        results.append(result)
    return sum(results)
...
def objective(trial):
    ...
    if w3 < 0:
        return float('inf')
    return evaluate_problems(w1, w2, w3)
    

if __name__ == "__main__":
    study = optuna.create_study(direction='minimize')
    study.optimize(objective, n_jobs=8, n_trials=200)
